    ...
    socket.on('join_room', (payload) => {
        ...
        io.in(room).fetchSockets().then((sockets)=>{
            ...
            else
            {
                ...
                /* Announce to everyone who in the room who else is in the room*/
    ...
    socket.on('invite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets)=>{
            ...
            if((typeof sockets == 'undefined') || (sockets===null) || !sockets.has(requested_user)) 
            {
                response = {
                    result: 'fail',
                    message: 'The user that invited is no longer in the room'
                }
                socket.emit('invite_response', response);
                serverLog('invite command failed', JSON.stringify(response));
                return;
    ...
    socket.on('uninvite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets)=>{
            ...
            if((typeof sockets == 'undefined') || (sockets===null) || !sockets.has(requested_user)) 
            {
                response = {
                    result: 'fail',
                    message: 'The user that uninvited is no longer in the room'
                }
                socket.emit('uninvited', response);
                serverLog('uninvite command failed', JSON.stringify(response));
                return;
...
function flip_line(who,dr,dc,r,c,board) {
     ...
     //see if proposed token to flip is off the board
    if(( r + dr < 0) || ( r + dr > 7)) {
        return false;
    }
    if(( c + dc < 0) || ( c + dc > 7)) {
    ...
    io.of('/').to(game_id).allSockets().then((sockets) => {
     ...
     if (legal_moves === 0) {
        ...
        if (whitesum > blacksum) {
            winner = "white"; //may change name here for different tokens
        }
        if (whitesum < blacksum) {
            winner = "black";
        }
